...
class GRUModel_improve(nn.Module):
    def __init__(self, model_config, device):
        super(GRUModel,self).__init__()
        # Debug
        model_config['feature_size'] = 10
        ...
        self.lstm = nn.ModuleList(self._create_lstm_cell(
            self.feature_size, 
            self.hidden_units,
            self.num_layers
        ))
    ...
    @staticmethod
    def _create_lstm_cell(input_size, hidden_units, num_layers):
        lstm = [nn.LSTM(input_size, hidden_units, bidirectional=True)] # First layer
        lstm += [nn.LSTM(hidden_units*2, hidden_units, bidirectional=True) for _ in range(num_layers-1)] # 2 -> num_layers
        return lstm
    ...
    def forward(self, x):
        ...
        lstm_inputs = []
        for index, linear in enumerate(self.linear_feature_list):
            lstm_input = linear(x[:, index].float())
            lstm_inputs.append(lstm_input)
        lstm_inputs = torch.stack(lstm_inputs)
        outputs, _ = self._compute_lstm(self.lstm, lstm_inputs, batch_size)
        logit_list = []
        for index, (t_fw, t_bw) in enumerate(zip(self.output_points_fw, self.output_points_bw)):
            lstm_output = []
            if t_fw is not None and not math.isnan(t_fw):
                lstm_output.append(outputs[int(t_fw), :, :self.hidden_units]) 
            if t_bw is not None and not math.isnan(t_bw):
                lstm_output.append(outputs[int(t_bw), :, self.hidden_units:])
            lstm_output = torch.cat(lstm_output, dim=1).to(self.device)
            logit = self.list_linear[index](lstm_output)
            logit_list.append(logit)
        # logit_list = torch.stack(logit_list)
        # print(logit_list.shape)
        # lstm = self.tmp(logit_list)
        return logit_list
    ...
    def _compute_lstm(self, LSTMs, _input, batch_size):
        hidden = self.init_hidden(batch_size)
        for i, lstm in enumerate(LSTMs):
            output, state = lstm(_input, hidden)
            output = torch.transpose(self.batch_norm_list[i](torch.transpose(output,1,2)),1,2)
            if i > 0:
                _input = _input + output
            else:
